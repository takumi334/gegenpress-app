...
export async function GET(req: NextRequest, context: { params: Promise<{ threadId: string }> }) {
  ...
  if (!threadId) {
    return NextResponse.json({ error: "threadId is required" }, { status: 400, headers: THREAD_DETAIL_CACHE_HEADERS });
  }
  const index = Number(threadId);
  if (isNaN(index) || index <= 0) {
    return NextResponse.json({ error: "invalid threadId" }, { status: 400, headers: THREAD_DETAIL_CACHE_HEADERS });
  }
  ...
  if (!thread) {
    return NextResponse.json({ error: "thread not found" }, { status: 404, headers: THREAD_DETAIL_CACHE_HEADERS });
  }
  ...
  return NextResponse.json(
    ...
    { headers: THREAD_DETAIL_CACHE_HEADERS }
  );
}
